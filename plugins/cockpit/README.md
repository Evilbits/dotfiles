...
While snoozed, the session sits in the snoozed section of the picker showing the remaining time or the MR it waits on, the status line shows the same, and in tmux the session's window is renamed `⏾ Read-protected stash fields` (`⏰` once due) until the snooze clears, when its own name comes back. Every tmux window running a session is named after the session's subject the same way, refreshed by the minute job and whenever you prompt or snooze, so the window list reads as work (`tmux_window_names` turns this off, leaving only the snooze marks). A background session shown with `claude attach` counts as running in the pane that shows it, so Enter jumps there and its snooze mark lands on that window. Every minute a background job checks the clock and the watched MRs. When a snooze fires you get a macOS notification naming what happened, `!17005: 2 new comments (jane.doe)` for instance, and clicking it opens the session. The session moves to the top of the picker marked `⏰`, every status line shows a due count, and if you had closed the session it is reopened in tmux. Opening it, from the picker or the notification, leaves the snooze in place, so you can look at a parked session without losing its watch. Typing into the session clears it (housekeeping commands such as `/exit`, `/clear` or a new `/snooze` do not count; the list is `snooze_keep_commands` in the config): a prompt hook removes the snooze and puts a line in the chat saying what fired, `unsnoozed, it fired: !17005 2 new comments (jane.doe) · waiting for review`, or that nothing has happened yet.

From a shell the same commands are `cockpit --snooze current …` and `cockpit --unsnooze current`, where `current` is the session in this tmux pane; `cockpit --due` lists snoozes and `cockpit --wake` runs the check by hand.

When cockpit files a session under the wrong ticket, because a key came up in passing, or under a ticket when it has none, `/ticket PROD-1234` or `/ticket none` inside the session pins it; from a shell, `cockpit --ticket current PROD-1234`. The pin wins over the prompts, the title and the branch everywhere the ticket shows. At the start of every session a banner lists what is snoozed.

## The status line

![status line](docs/statusline.png)

`model · effort | repo | ticket · subject | snooze | ⏰ N due | branch [primary|worktree] | !MR title`. The ticket is the one the picker filters on, so it is there even when the name leaves it out, and it is a link to the ticket in Jira in terminals that support hyperlinks; the host is `jira_host` in the config, or the host of the first Jira link the session was given. The subject is the picker's, so a `/rename` shows up here too. A snoozed session shows its wake time or the MR it waits on, dimmed; when any snooze is due a yellow count appears in every session. The merge requests that came out of the session, every one it dealt with that you authored, newest first and at most three, or failing that the one for the current branch, appear after the branch as links you can click in terminals that support hyperlinks: the newest with its title, the older ones by number, each with `· merged` or `· closed` once it is no longer open; the lookup is served from a cache the minute job refreshes, so the status line never waits on GitLab. Setup adds the setting to `~/.claude/settings.json`:
