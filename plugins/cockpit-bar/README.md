...
**In the dropdown.** Four sections: **Due** (snoozes that fired), **Running** (open sessions, the ones working first, each with its activity and timer on the right), **Snoozed** (what they wait for on the right) and **Recent** (the last eight closed sessions). Every row is the kind of work as a pill (Review, Implement, Design, Debug, from the skill the session started with, `skill_verbs` in cockpit's config) and the subject without identifiers, the same as cockpit's picker; the ticket and the MRs are in the flyout. Clicking a row runs cockpit's open: it jumps to the session's tmux pane, or resumes a closed one in a new tmux window, and brings the terminal to the front.
...
`docs/demo-provider` prints made-up rows, and `COCKPIT_BAR_PROVIDER` and `COCKPIT_BAR_STATE` point a second instance at it and at an empty state folder, which is how the screenshot above was taken without any real session in it.
